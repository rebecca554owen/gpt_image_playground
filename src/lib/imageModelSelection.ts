...
import { LEGACY_IMAGES_MODEL, FOUR_K_IMAGES_MODEL } from './apiProfiles'
...
export function getImageModelForSize(profile: ApiProfile, size: string) {
  ...
  return isFourKImageSize(size) ? FOUR_K_IMAGES_MODEL : LEGACY_IMAGES_MODEL
}

export function isSizeManagedImageProfile(profile: ApiProfile) {
  return profile.provider === 'openai' && profile.apiMode === 'images'
    && (profile.model === LEGACY_IMAGES_MODEL || profile.model === FOUR_K_IMAGES_MODEL)
}
