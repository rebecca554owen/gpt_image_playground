...
function mapFalEndpoint(model: string, isEdit: boolean): string {
  ...
  if (normalized.includes('gpt-image-2.5/')) {
    const base = normalized.replace(/\/(edit|text-to-image)$/, '')
    return `${base}/${isEdit ? 'edit' : 'text-to-image'}`
  }
...
function mapFalQuality(quality: TaskParams['quality'], model: string): Exclude<TaskParams['quality'], 'auto'> {
