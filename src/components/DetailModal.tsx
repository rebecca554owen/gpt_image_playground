        ...
        <div className="md:w-1/2 w-full h-64 md:h-auto bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
          ...
          {task.status === 'error' && isReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <p className="text-sm font-medium text-yellow-500">{isServerJobReconnecting ? '正在恢复服务端任务' : '重连中'}</p>
            </div>
          )}
          {task.status === 'error' && !isReconnecting && (
            <div className="w-full max-w-md px-4 text-center">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                ...
                <div className="relative group">
                  <button
                    ...
                    aria-label={task.serverJobIds?.length ? '重新读取已有结果' : '重试任务'}
                  >
                  ...
                  <ViewportTooltip visible={retryTooltip.visible} className="whitespace-nowrap">
                    {task.serverJobIds?.length ? '重新读取已有结果' : '重试任务'}
                  </ViewportTooltip>
                </div>
              </div>
            </div>
