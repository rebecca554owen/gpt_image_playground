...
export default function ImageModelPicker({ value, onChange, allowDefault = false, compact = false, showSummary = false }: Props) {
  ...
  return (
    <>
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">{showSummary && selected ? `${selected.shortName}${selected.badge ? ` · ${selected.badge}` : ''}` : compact ? label : selected?.name || label}</span>
