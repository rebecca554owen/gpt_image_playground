...
test('结果图片域名仅匹配精确主机或配置的子域后缀', () => {
  const rules = new Set(['imagefil.scdn.app', '*.000000033.xyz', 'ztyunjuan.com'])
...
test('部署配置使用 secret 文件、回环端口和可信 Docker 代理链', () => {
  ...
  assert.match(compose, /IMAGE_JOB_RESULT_IMAGE_HOSTS: imagefil\.scdn\.app,\*\.000000033\.xyz,img\.sulmes\.com,download1\.bnq777\.xyz,ztyunjuan\.com/)
