...
test('结果图片域名仅匹配精确主机或配置的子域后缀', () => {
  ...
  assert.equal(isAllowedResultImageHost('fakeztyunjuan.com', rules), false)
  assert.equal(isAllowedResultImageHost('klong.lat', rules), false)
  assert.equal(isAllowedResultImageHost('cdn.image.klong.lat', rules), false)
  assert.equal(isAllowedResultImageHost('fakeimage.klong.lat', rules), false)
  assert.equal(isAllowedResultImageHost('cdn.rolldek.com', rules), false)
  assert.equal(isAllowedResultImageHost('fakerolldek.com', rules), false)
...
test('部署配置使用 secret 文件、回环端口和可信 Docker 代理链', () => {
  ...
  assert.match(compose, /IMAGE_JOB_RESULT_IMAGE_HOSTS: imagefil\.scdn\.app,\*\.000000033\.xyz,img\.sulmes\.com,download1\.bnq777\.xyz,ztyunjuan\.com,image\.klong\.lat,rolldek\.com/)
...
test('Service Worker 不缓存任务与代理接口并强制检查新版本', () => {
  ...
  const restrictedCaching = serviceWorker.indexOf('!APP_SHELL_URLS.has(url.href) && !url.pathname.startsWith(ASSETS_PATH)')
  assert.notEqual(networkOnly, -1)
  assert.ok(cacheFirst > networkOnly)
  assert.ok(restrictedCaching > networkOnly)
